@props(['variant' => 'default'])

@php
$baseClass = "inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2";

$variants = [
    'default' => 'border-transparent bg-primary text-primary-foreground hover:bg-primary/80',
    'secondary' => 'border-transparent bg-secondary text-secondary-foreground hover:bg-secondary/80',
    'destructive' => 'border-transparent bg-destructive text-destructive-foreground hover:bg-destructive/80',
    'outline' => 'text-foreground',
    'prospek' => 'border-transparent bg-blue-500 text-white hover:bg-blue-600',
    'negosiasi' => 'border-transparent bg-yellow-500 text-white hover:bg-yellow-600',
    'aktif' => 'border-transparent bg-green-500 text-white hover:bg-green-600',
];

$classes = $baseClass . ' ' . $variants[$variant];
@endphp

<div {{ $attributes->merge(['class' => $classes]) }}>
    {{ $slot }}
</div>
